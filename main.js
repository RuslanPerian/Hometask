...
//Число вопросов, которое будет использоваться

const NumberOfAllQuestion = questions.length; //если нужны все вопросы то = questions.length, если вопросы нужны не все то указываем = нужное число вопросов (например, const NumberOfAllQuestion = N, где 0 < N <= questions.length и N - нужное количество вопросов)

//Общее число вопросов

numberOfAllQuestions.innerHTML = NumberOfAllQuestion;
...
const quizOver = () => {
    ...
    numberOfAllQuestions2.innerHTML = NumberOfAllQuestion;
};
...
const randomQuestion = () => {
    let randomNumber = Math.floor(Math.random() * NumberOfAllQuestion);

    //якорь для одинаковых вопросов

    let hitDuplicate = false;

    if (indexOfPage == NumberOfAllQuestion) {
        quizOver();
    } else {
...
const answerTrackerGenerator = () => {
    for (let i = 0; i < NumberOfAllQuestion; i++) {
